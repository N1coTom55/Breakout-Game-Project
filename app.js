...
// Ajuste de la proporción de altura
...
function drawBall() {
  ball.tail.forEach((pos, index) => {
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, ball.size * (1 - index * 0.05), 0, Math.PI * 2);
    ctx.fillStyle = `rgba(255, 0, 0, ${1 - index * 0.1})`;
    ctx.fill();
    ctx.closePath();
  });
...
function moveBall() {
  ball.tail.unshift({ x: ball.x, y: ball.y });
  if (ball.tail.length > 10) ball.tail.pop();

  ball.x += ball.dx;
  ball.y += ball.dy;

  // Detectar colisión con los muros
  ...
  // Reiniciar si la bola cae al fondo
